...
if cargo run --locked --quiet -p criv-perf-harness --bin criv-perf-harness -- \
...
run_harness() {
  cargo run --locked --quiet -p criv-perf-harness --bin criv-perf-harness -- \
    --binary "$fake_binary" \
    --profile test \
    --allow-non-release \
    --samples 1 \
...
cargo run --locked --quiet -p criv-perf-harness --bin criv-perf-report -- \
